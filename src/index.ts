...
export function docs(config: DocsConfig) {
    ...
    for (const doc of allDocs) {
        ...
        if (routePath.endsWith('/index')) {
            routeMap.set(routePath.replace(/\/index$/, '') || basePath, doc)
        }
    }

    app.get('/*', async (c) => {
        let currentPath = new URL(c.req.url).pathname.replace(/\/$/, '') || basePath
        ...
        const title = doc.meta.title || path.basename(doc.filePath, '.md')
...
export type { DocsConfig, NavItem, ParsedDoc, DocMeta } from './types'